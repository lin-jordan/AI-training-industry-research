...
type ModuleContentProps = {
  ...
  sources: Source[];
  supplementalContent?: Readonly<Partial<Record<string, ReactNode>>>;
};

function renderBlock(block: EducationalModule["sections"][number], sources: Source[]) {
  if (block.type === "paragraph") {
    return (
      <section className="module-prose" id={block.id}>
  ...
  if (block.type === "callout") {
    return (
      <aside className="module-callout" id={block.id}>
  ...
  if (block.type === "details") {
    return (
      <details className="module-details" id={block.id}>
  ...
  return (
    <section className="module-stage" id={block.id}>
...
export function ModuleContent({ module, sources, supplementalContent = {} }: ModuleContentProps) {
  return (
    <div className="module-content">
      {module.sections.map((block) => (
        <Fragment key={block.id}>
          {renderBlock(block, sources)}
          {supplementalContent[block.id]}
        </Fragment>
      ))}
    </div>
  );
